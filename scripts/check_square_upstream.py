...
def main():
    ...
    if not link.exists():link.symlink_to('image_abs.hdf5')
    torch.set_num_threads(4);checks={}
    for m in ['image','point_ae']:
        cfg,original=config(m)
        ...
        torch.save(dict(model={k:v.detach().cpu() for k,v in p.state_dict().items()},config=dict(modality=m,seed=42,epochs=100,smoke=False),epoch=0),folder/'policy.pt')
