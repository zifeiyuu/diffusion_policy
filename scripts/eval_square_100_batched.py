...
from square_dp_data import make_policy
...
class Sim:
    def __init__(self):
        ...
        self.process=subprocess.Popen([sys.executable,str(Path(__file__).with_name('square_dp_native_worker.py')),self.listener.address],stdout=self.log,stderr=self.log,env={**os.environ,'OPENBLAS_NUM_THREADS':'1','OMP_NUM_THREADS':'1'})
...
def load(run):
    path=Path(run)/'final_ema.pt';payload=torch.load(path,map_location='cpu');cfg=payload['config']
    assert cfg['modality'] in ['image','point_ae'] and cfg['seed']==42 and cfg['epochs']==100 and payload['epoch']==99 and not cfg['smoke']
    policy=make_policy(cfg['modality']);policy.load_state_dict(payload['model'],strict=True);policy.cuda().eval()
    if cfg['modality']=='point_ae':
        ...
        def with_ae(ob):
            ...
            data={k:ob[k] for k in ROBOT};data['point_ae']=ae(points.reshape(-1,256,2)).reshape(B,T,128)
            return original(data)
        policy.predict_action=with_ae
    info=dict(run=str(run),training_seed=42,epoch=99,modality=cfg['modality'],checkpoint=str(path),checkpoint_sha256=sha(path),config=cfg,
        python_executable=sys.executable,working_directory=os.getcwd(),evaluation_source_sha256=sha(Path(__file__)),worker_sha256=sha(Path(__file__).with_name('square_dp_native_worker.py')))
...
def evaluate(a):
    ...
    def report(complete=False):
        r=dict(complete=complete,smoke=bool(a.limit),run=a.run,modality=modality,training_seed=info['training_seed'],checkpoint_epoch=info['epoch'],
            ...
            training_wall_seconds=json.loads((Path(a.run)/'complete.json').read_text())['total_trainer_wall_seconds'],task_makespan=task_metrics(completed),
    ...
    try:
        ...
        while next_episode<count or slots:
            for si,sim in enumerate(sims):
                ...
                ob=sim.recv();assert ob.pop('delta_controller') and not ob['success']
            ...
            assert raw.shape==(len(active),8,7) and np.isfinite(raw).all()
            assert np.max(np.abs(raw))<1.001
            env_action=np.clip(raw,-1,1)
            ...
            for step in range(8):
                ...
                for si in current:
                    ...
                    if ob['success'] or ob['done'] or len(s['actions'])>=a.max_steps:
                        ...
                        np.savez_compressed(folder/f'trajectory_{s["i"]:03d}.npz',actions=s['actions'],raw_delta7=s['raw'],robot=s['robot'],replan_steps=s['replans'])
                        e=dict(episode=s['i'],demo_name=names[s['i']] if a.split=='valid' else None,success=bool(ob['success']),steps=len(s['actions']),seconds=wall,execution_wall_seconds=wall,
                            initial_state_max_abs_error=s['restore_error'],delta_controller_verified=True,simulator_runtime=s['runtime'],video=f'episode_{s["i"]:03d}.mp4' if s['i']<3 else None)
