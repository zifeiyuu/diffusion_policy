...
D=Path(__file__).resolve().parents[1];O=Path('/home/zxiao93/Documents/LaDiWM/results/dp_transformer100_20260923')
...
def report():
    with (O/'report.log').open('a') as f:subprocess.run([sys.executable,'scripts/report_square_transformer.py'],stdout=f,stderr=subprocess.STDOUT,check=True)
def run(name,args):
...
def main():
    ...
    try:
        ...
        while not (previous/'published_commit.txt').exists():
            ...
            time.sleep(45)
        subprocess.run([sys.executable,'scripts/publish_square_transformer.py','--init'],check=True)
        run('preflight',['scripts/check_square_transformer.py'])
        for m in ['image','point_ae']:
            run(m+'_train',['scripts/train_square_transformer.py','--modality',m])
            root=O/f'{m}_seed42'
        ...
        with (O/'publish.log').open('a') as f:
            subprocess.run([sys.executable,'scripts/publish_square_transformer.py'],stdout=f,stderr=subprocess.STDOUT,check=True)
