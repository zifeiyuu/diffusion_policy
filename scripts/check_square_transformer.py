...
ROOT=Path('/home/zxiao93/Documents/LaDiWM/results/dp_transformer100_20260923')
def main():
    torch.set_num_threads(4);checks={}
    for m in ['image','point_ae']:
        cfg,original=config(m,architecture="transformer")
        ...
        torch.save(dict(model={k:v.detach().cpu() for k,v in p.state_dict().items()},config=dict(modality=m,seed=42,epochs=100,smoke=False,architecture="transformer"),epoch=0),folder/'policy.pt')
