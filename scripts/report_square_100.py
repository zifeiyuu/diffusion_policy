...
import json,os,sys,re,shutil,statistics
...
def main():
    ...
    for m in ['image','point_ae']:
        ...
        for split,e in zip(['valid','random_saved'],ev):
            ...
            a=e['task_makespan']['success_sim_seconds'];duration.append(f"| {m} | {split} | {a['count']} | {fmt(a['mean'])} / {fmt(a['median'])} / {fmt(a['p95'])} | {fmt(e['amortized_sampling_ms_per_episode_call'])} / {fmt(statistics.median(e['single_env_warm_sampling_ms']))} | {fmt(e.get('frozen_ae_total_seconds'))} | {fmt(e['peak_cuda_allocated_bytes']/2**20)} |")
        ...
        for name in ['config.json','progress.json','complete.json','epochs.jsonl','evaluation_provenance.json']:
            if (run/name).exists():shutil.copy2(run/name,dest/name)
    for source in [O/'native_preflight.json',O/'cache/manifest.json',O/'cache/point_ae_manifest.json']:
        if source.exists():shutil.copy2(source,R/'artifacts'/source.name)
    ...
    costs='Train wall includes setup, validation and checkpointing; summed training-epoch compute remains in JSON. Eval wall includes model/env startup, reset, sampling, simulation, videos and artifact saving. Evaluation batches up to8 environments. Timed policy calls include image encoding or frozen AE encoding of both observed point frames, DDPM100 and output transfer. AE cost is included in policy sampling, not separately isolated (NR in the table). Batch-amortized latency is not single-env latency; warm B1 is the median of3 measured calls. Each evaluator verifies batch1 sampling bit-for-bit against the upstream sampler. Independent episode generators use seed42+i. Success makespan is steps/20Hz; failed episodes are censored at400 steps. Peak GPU is PyTorch allocation. Original AE pretraining cost is unrecorded; no monetary/energy measurement. Cached data/features are reused; cache preparation, if performed, is distinct from per-run training.'
