...
def main():
 ...
 body='''Question: with action diffusion near KUBM Stage2 size, compare image DP and frozen PointAE DP at51 and100 completed epochs.

Only action UNet widths change from[512,1024,2048] to[88,176,352]. Action diffusion has9,342,242 trainable parameters vs KUBM no-future Stage2's9,167,886 (+1.90%). Image retains22,394,176 trainable encoder parameters, so total trainable image DP=31,736,418. PointAE DP has9,342,242 trainable parameters plus reused frozen170-parameter AE encoder (full supplied AE encoder+decoder=2,540). This matches action-network parameter count, not FLOPs or full-model size; KUBM Stage2 count includes unused modules. No KUBM component is inserted into DP.

Seed42 for each modality;180/20 masks and saved Random50 paired initial states unchanged. Absolute10D actions, horizon16, observe2, execute8, batch64, AdamW1e-4, original3050-epoch cosine schedule/warmup500, EMA, DDPM100 and image crop/normalization unchanged. Image encoder is jointly trained; AE is frozen. Train from scratch for100 epochs; skip initial rollout. At51 completed epochs evaluate Eval20 and Random50; save the periodic full checkpoint. Save final full model+EMA+optimizer and evaluate both sets at100 epochs. The two fixed endpoints are reported without replacing one by the higher SR. Single-seed results do not establish statistical significance.

Policy/training use unchanged robodiff. Simulator uses dp_eval141 (robosuite1.4.1), restoring full saved XML and state. Absolute training labels reuse the previous upstream converter output generated in robodiff1.2; no data conversion change in this architecture ablation. Dataset caches and AE preprocessing are reused. Previous full-size DP runs remain intact.
...
'''
 ...
 (R/'README.md').write_text('# Small action-diffusion DP: image and PointAE,51/100 epochs\n\n'+body)
 register_report(R/'README.md',entry=body)
 p=L/'experiment_logs/summary/README.md';s=p.read_text();a='<!-- dp-small100:start -->';b='<!-- dp-small100:end -->'
 block=a+'\n## Small action-diffusion DP:51/100 epochs\n\n'+body+'\n[Evidence](../2026-09-23/dp_small100/README.md).\n'+b
