...
Additional modality: `point_ae` uses the bundled14KB frozen LaDiWM/K-Steer AE and cached128D absolute-point features concatenated with robot9. There is no trainable observation MLP in this branch. See `assets/point_ae/README.md` for exact input semantics and pretraining provenance, and `scripts/SKYNET.md` for commands. Original image/point_flow branches are unchanged.
