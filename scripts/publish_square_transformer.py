...
def outside(name):
    s=(L/name).read_text()
    if '/summary/' in name:a,b='<!-- dp-transformer100:start -->','<!-- dp-transformer100:end -->'
    else:
        key='experiment_logs/2026-09-23/dp_transformer100/README.md';a,b=f'<!-- experiment:{key}:start -->',f'<!-- experiment:{key}:end -->'
...
def main():
    ...
    for m in ['image','point_ae']:
        ...
        assert (policy_cfg.base.n_layer if m=='point_ae' else policy_cfg.n_layer)==8
    ...
    subprocess.run(['git','-C',str(L),'commit','-m','Report original Transformer DP at51 and100 epochs'],check=True)
