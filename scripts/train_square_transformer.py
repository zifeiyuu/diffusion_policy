...
def main():
    ...
    root=Path('/home/zxiao93/Documents/LaDiWM/results')/'dp_transformer100_20260923'/f'{a.modality}_seed42'
    root.mkdir(parents=True,exist_ok=True)
    if (root/'complete.json').exists():return
    dims=None
    cfg,original=config(a.modality,architecture="transformer")
    ...
    started=time.perf_counter();workspace=TrainDiffusionTransformerHybridWorkspace(cfg,output_dir=str(root));workspace.run()
    ...
    torch.save(dict(model={k:v.detach().cpu() for k,v in workspace.ema_model.state_dict().items()},config=dict(modality=a.modality,seed=42,epochs=100,smoke=False,down_dims=dims,architecture="transformer"),epoch=99),endpoint/'policy.pt')
    choices=[]
